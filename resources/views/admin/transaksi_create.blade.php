...
<head>
    ...
    <title>Tambah Transaksi (Admin) - Pet Hotel</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap" rel="stylesheet">
</head>
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <h1 class="text-2xl font-bold mb-6 text-indigo-700 flex items-center gap-2"><span>Tambah Transaksi</span> <span>💰</span></h1>
        <form action="{{ route('admin.transaksi.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select name="pelanggan_id" id="pelanggan_id" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-200 focus:outline-none" required onchange="updateHewanOptions()">
                    ...
                    @foreach($pelanggans as $pelanggan)
                        <option value="{{ $pelanggan->id }}" data-hewans='@json($pelanggan->hewans)'>{{ $pelanggan->nama }} ({{ $pelanggan->email }})</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <div class="space-y-2">
                    @foreach($layanans as $layanan)
                        <label class="flex items-center">
                            <input type="checkbox" name="jenis_layanan[]" value="{{ strtolower($layanan->nama) }}" data-harga="{{ $layanan->harga }}" class="mr-2" onchange="handleLayananChange()">
            ...
            <div>
                ...
                <input type="date" name="tanggal_masuk" id="tanggal_masuk" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-200 focus:outline-none" required onchange="handleTanggalChange()">
            </div>
            <div id="tanggal_keluar_container">
                <label class="block text-sm font-semibold mb-1">Tanggal Keluar</label>
                <input type="date" name="tanggal_keluar" id="tanggal_keluar" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-200 focus:outline-none" onchange="handleTanggalChange()">
            ...
            <div>
                ...
                <input type="number" name="total_harga" id="total_harga" min="0" step="0.01" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-200 focus:outline-none" required readonly>
            </div>
            <div>
            ...
            <div id="jumlah_dibayar_container">
                ...
                <input type="number" name="jumlah_dibayar" id="jumlah_dibayar" min="0" step="0.01" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-200 focus:outline-none" onchange="hitungSisa()">
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-200 focus:outline-none"></textarea>
            </div>
